...
for(var i = 0; i < set.num_of_batt; i++){
    (function(bound_i) {
        ...
        ports_bat[i] = new SerialPort(set.port_batt[i], {baudRate: 9600}, function (err) {
            if (err) {
...
function bms_parser(batt1_accu, batt1_command, index) {
    ...
    if (batt1_command == 3) {
        ...
        for (let i = 1; i <= values_bat[index].Battery_serial; i++) {
            ...
            //values_bat.Residual_capacity += values_bat.Residual_capacity[index]; // 배터리 용량 계산 [21.10.14]
            //values_bat.Residual_capacity * 50;
        values_bat[index].Alive = 0;
